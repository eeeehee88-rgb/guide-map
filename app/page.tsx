...
function mercatorWorldPoint(lat:number, lng:number) {
...
function staticMapPixelPosition(point:{lat:number;lng:number}, center:{lat:number;lng:number}, zoom:number, imageWidth:number, imageHeight:number) {
  const scale = 2 ** zoom;
  const centerPx = mercatorWorldPoint(center.lat, center.lng);
  const pointPx = mercatorWorldPoint(point.lat, point.lng);
  const leftPct = (imageWidth/2 + (pointPx.x - centerPx.x) * scale) / imageWidth * 100;
  const topPct = (imageHeight/2 + (pointPx.y - centerPx.y) * scale) / imageHeight * 100;
...
export default function Home() {
  ...
  const saveTripProfile = () => {
    ...
    setSheet("places");
    setSheetCollapsed(false);
    setGuideRecommendations((current)=>current.length ? current : spots);
  };
        ...
        {sheet === "places" && (
          <>
            ...
            {selected.aiReason && <div className="ai-place-card">
              ...
              <small className="ai-disclaimer">AI가 등록된 장소 정보와 여행 구성으로 정리한 추천 정보예요. 가격·메뉴·영업시간은 방문 전에 확인해 주세요.</small>
            </div>}
                </div>
              </section>
            </div>,document.body)}
      ...
      {guideOpen && (
        <section className="guide-overlay">
          ...
          <div className="guide-scroll">
            ...
            {aiGuideLoading ? <div className="ai-guide-loading"><Sparkles size={30}/><b>이미지 가이드북을 불러오고 있어요</b><span>Codex CLI에서 만든 개발용 결과물을 확인하는 중입니다.</span></div> : staticGuidebook ? <div className="travel-guide static-guidebook" ref={guideRef}>
              <header>
