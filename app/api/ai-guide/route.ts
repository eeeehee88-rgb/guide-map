export async function POST(request:Request) {
  ...
  const response = await fetch("https://api.deepseek.com/chat/completions",{
    ...
    body:JSON.stringify({
      ...
      max_tokens:4500,
      messages:[
        {role:"system",content:`너는 일본 가족여행 일정 설계자다. 제공된 장소만 사용하고, 아이와 고령자가 무리하지 않도록 하루 방문 수와 휴식을 조절한다. 이동 동선은 좌표와 카테고리를 참고한다. 한국어 JSON {"title":"...","overview":"...","days":[{"day":1,"title":"...","stops":[{"id":"장소 id","time":"09:30","reason":"방문·메뉴·휴식 포인트"}],"tips":["..."]}],"familyTips":["..."],"weatherBackup":["..."]}만 출력한다. 여행 일수가 없으면 1일 코스로 만든다.`},
