export async function POST(request:Request) {
  ...
  const response=await fetch("https://api.deepseek.com/chat/completions",{
    ...
    body:JSON.stringify({
      ...
      max_tokens:800,
      messages:[
        {role:"system",content:`너는 선택 국가의 현지 매장 정보를 한국 가족 여행자 관점으로 분석하는 AI 여행 큐레이터다. 입력된 Google 매장명·업종·설명·후기만 근거로 한국어 JSON을 만든다. 과장하지 말고, 근거가 약하면 "확인 필요"라고 쓴다.
...
형식 {"title":"AI 분석 제목 18자 내외","decision":"왜 추천/주의인지 한 문장","description":"무엇을 파는 곳인지와 여행자가 기대할 경험","items":[{"name":"메뉴·상품명 또는 판매 품목 종류","price":"실제 또는 추정 가격"}],"visitTip":"가족 여행자가 방문할 때의 실전 팁","evidenceTags":["후기 기반","영업정보 확인","가족 동선","가격 추정"] 중 실제 근거를 2~4개 자연어 태그로 작성,"source":"Google 매장 설명·후기 / Google 업종·일반 시세 추정 중 해당 근거"}.`},
